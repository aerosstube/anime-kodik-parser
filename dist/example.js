...
async function main() {
    try {
        ...
        const token = await index_1.KodikParser.getToken();
        console.log('Токен получен:', token);
        console.log('\n--- Создание парсера ---');
        const parser = await (0, index_1.createParser)(token);
        console.log('\n--- Поиск аниме ---');
            ...
            if (firstResult.shikimori_id) {
                ...
                if (info.translations.length > 0) {
                    ...
                    if (info.series_count > 0) {
                        console.log('\n--- Получение ссылки ---');
                        const [link, quality] = await parser.getLink(firstResult.shikimori_id, 'shikimori', 1, info.translations[0].id);
        ...
        const search = await (0, index_1.createSearch)(token);
        ...
        const list = await (0, index_1.createList)(token);
